...
class CNF(nn.Module):
    ...
    def forward(self, t: Tensor, x: Tensor, **kwargs) -> Tensor:
        ...
        with record_function("Neural_Network_Call"):
            out = self.network(t, x, **kwargs)
        return out
    ...
    def decode(self, z: Tensor, num_timesteps: int = 200, **kwargs) -> Tensor:
        self.__calculate_block_mask(kwargs)
        with record_function("decode/solver"):
            sol = self.solver(self, z, 1.0, 0.0, num_timesteps, **kwargs)
        return sol
    ...
    def sample(
        self, shape: tuple[int, ...], num_timesteps: int = 200, **kwargs) -> Tensor:
        
        z = torch.randn(*shape, device=self.frequencies.device, dtype=self.frequencies.dtype)
        with record_function("Decode_Step"):
            dec = self.decode(z, num_timesteps, **kwargs)
