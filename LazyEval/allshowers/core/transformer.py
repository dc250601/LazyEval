...
class FlexEncoderLayer(nn.Module):
    ...
    def forward(
        ...
        with record_function("MHA"):
            mha = self.multihead_attention(x, mask=mask)
        x = self.layer_norm1(x + mha)
        with record_function("MLP"):
            mlp = self.feedforward(x)
        x = self.layer_norm2(x + mlp)
        return x
...
class Transformer(nn.Module):
    ...
    def forward(
        ...
        with record_function("Conditioning"):
        ...
        with record_function("TransformerHead"):
            x = self.head(x)
        return x
